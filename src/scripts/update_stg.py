...
def main():
    spark = (
            ...
            .builder
            .master('yarn')
            .appName(f"{user}_update_stg_by_date_{date}_with_depth_{depth}")
            .getOrCreate()
        )
    ...
    def parquet_event(date, depth, user, hdfs_path, geo_path): 
        ...
        for i in range(int(depth)):
            ...
            events.write.mode('overwrite').partitionBy('event_type').parquet(f'{hdfs_path}/user/{user}/data/events/date={i_date}')
